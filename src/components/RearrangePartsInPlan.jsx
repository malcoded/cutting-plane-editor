...
function groupCells(rowParts) {
  ...
  cells.forEach(
    (c) =>
      (c.xEnd = Math.max(...c.parts.map((pt) => toNum(pt.x) + toNum(pt.width))))
  );
...
function countInternalCuts(cell, row, cuts) {
  return cuts.reduce((acc, cut) => {
    ...
    const x1 = toNum(cut.x1),
      x2 = toNum(cut.x2);
    const y1 = toNum(cut.y1),
      y2 = toNum(cut.y2);
    const inX = x1 >= cell.xStart - EPS && x2 <= cell.xEnd + EPS;
    const inY = y1 >= row.yStart - EPS && y2 <= row.yEnd + EPS;
...
function rearrangePlan(plan) {
  ...
  rows.forEach((row) => {
    ...
    row.vertCuts = cuts.filter((ct) => {
      ...
      const intersects = !(yB <= row.yStart + EPS || yA >= row.yEnd - EPS);
  ...
  const topTrim =
    horizontals.map((c) => toNum(c.y1)).sort((a, b) => a - b)[0] ?? 0;
  ...
  rows.forEach((row) => {
    ...
    sheet.wastePart?.forEach((wp) => {
      const wy = toNum(wp.y);
      if (wy >= row.yStart - EPS && wy <= row.yEnd + EPS)
        wp.y = (wy + dy).toString();
    });
